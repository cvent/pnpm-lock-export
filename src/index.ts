...
import type { Dependencies, Dependency, PackageLock as PackageLockV1 } from './types/PackageLockV1.js';

export default async function (lockfileDir: string): Promise<PackageLockV1> {
  const lock = await readWantedLockfile(lockfileDir, { ignoreIncompatible: true });
  if (lock == null) throw new Error('pnpm lockfile not found');
  const latestSupportedLockfileVersion = 5.4;
  if (lock.lockfileVersion > latestSupportedLockfileVersion)
    console.warn(
